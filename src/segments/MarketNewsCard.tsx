...
const MarketNewsCard: React.FC<MarketNewsCardProps> = ({ 
  ...
  <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-4 flex flex-col">
    <h2 className="text-lg font-bold text-indigo-900 dark:text-white mb-2">Market News</h2>
    {/* Show news for currently visible stocks in portfolio */}
    {filteredAndSortedStocks.length > 0 ? (
      <div className="space-y-3">
        {filteredAndSortedStocks.slice(0, 3).map((stock, index) => (
          <div key={stock.symbol} className="border-b border-gray-100 dark:border-gray-700 pb-3 last:border-b-0">
            <div className="flex items-center gap-2 mb-2">
              <img src={getLogoUrl(stock.symbol)} alt={stock.symbol} className="w-6 h-6 rounded bg-white border shadow object-contain" onError={e => (e.currentTarget.src = 'https://images.pexels.com/photos/590020/pexels-photo-590020.jpeg?auto=compress&cs=tinysrgb&w=600')} />
            ...
            {news[stock.symbol] && news[stock.symbol].length > 0 ? (
              <div className="space-y-2">
                {news[stock.symbol].slice(0, 2).map((n, i) => (
                  <div key={i} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 border border-gray-100 dark:border-gray-700">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">{n.source}</span>
                      {n.isCompetitor && (
                        <span className="px-1.5 py-0.5 bg-amber-100 text-amber-700 dark:bg-amber-900 dark:text-amber-300 rounded-full text-xs font-semibold">Comp</span>
                      )}
                    </div>
                    <h4 className="font-semibold text-gray-900 dark:text-white text-sm mb-1 line-clamp-1">{n.title}</h4>
              ...
              <div className="space-y-2">
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2 border border-gray-100 dark:border-gray-700">
                  ...
                  <h4 className="font-semibold text-gray-900 dark:text-white text-sm mb-1 line-clamp-1">{stock.symbol} Market Analysis: Latest Updates</h4>
                  <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-xs">
                    <span className="font-semibold text-gray-700 dark:text-gray-300">Sentiment:</span>
                    <span className="px-1.5 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300">
                      Neutral (0.0)
                    </span>
        ...
        {filteredAndSortedStocks.length > 3 && (
          <div className="text-center pt-2">
            <span className="text-xs text-gray-500 dark:text-gray-400">
              +{filteredAndSortedStocks.length - 3} more stocks in portfolio
            </span>
          </div>
        )}
      </div>
